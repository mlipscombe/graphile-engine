...
function quacksLikePgClient(pgConfig: {}): boolean {
...
export function quacksLikePgPool(pgConfig: {}): boolean {
  // A diagnosis of exclusion
  if (!pgConfig || typeof pgConfig !== "object") return false;
...
const withPgClient = async (pgConfig: pg.Client | pg.Pool | string = process.env.DATABASE_URL, fn: (pgClient: pg.Client) => any) => {
  if (!fn) {
