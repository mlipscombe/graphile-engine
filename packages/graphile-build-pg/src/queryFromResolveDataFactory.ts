...
export default ((queryBuilderOptions: QueryBuilderOptions = {}) => (from: SQL, fromAlias: SQL | null | undefined, resolveData: DataForType, options: {
  ...
  const usesCursor: boolean = explicitlyUsesCursor && explicitlyUsesCursor.length > 0 || calculateHasNextPage && calculateHasNextPage.length > 0 || calculateHasPreviousPage && calculateHasPreviousPage.length > 0 || false;
  ...
  if (options.withPagination || options.withPaginationAsFields) {
    queryBuilder.setCursorComparator((cursorValue, isAfter) => {
      ...
      } else if (cursorValue[0] === "natural" && isSafeInteger(cursorValue[1]) && cursorValue[1] >= 0) {
        if (isAfter) {
