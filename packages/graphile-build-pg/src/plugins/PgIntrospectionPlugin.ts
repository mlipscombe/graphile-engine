  ...
  introspectionResults.class.forEach(klass => {
    ...
    if (klass.tags.foreignKey) {
      ...
      foreignKeys.forEach((fkSpecRaw, index) => {
        ...
        const foreignColumns: string[] = rawForeignColumns ? parseSqlColumn(rawForeignColumns, true) : null;
...
export default (async function PgIntrospectionPlugin(builder, {
  pgConfig,
  pgSchemas: schemas,
  pgEnableTags,
  ...
  async function introspect() {
    ...
    const introspectionResultsByKind = cloneResults((await persistentMemoizeWithKey(cacheKey, () => withPgClient(pgConfig, async pgClient => {
  ...
  builder.registerWatcher(async triggerRebuild => {
    ...
    if (!pgSkipInstallingWatchFixtures) {
      ...
      try {
        ...
        console.warn(chalk.yellow("Enable DEBUG='graphile-build-pg' to see the error"));
        debug(error);
