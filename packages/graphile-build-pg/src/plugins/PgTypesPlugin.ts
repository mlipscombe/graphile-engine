...
export default (function PgTypesPlugin(builder, {
  ...
  builder.hook("build", build => {
    ...
    */
    ...
    const {
      GraphQLJSON,
      GraphQLJson
    } = makeGraphQLJSONTypes(graphql); // pgExtendedTypes might change what types we use for things
    ...
    const oidLookup = {
      "20": stringType("BigInt", "A signed eight-byte integer. The upper big integer values are greater then the max value for a JavaScript number. Therefore all big integers will be output as strings and not numbers."),
    ...
    const reallyEnforceGqlTypeByPgTypeAndModifier = (type, typeModifier) => {
      ...
      if (!gqlTypeByTypeIdAndModifier[type.id][typeModifierKey] && type.type === "r") {
        ...
        pg2GqlMapper[type.id] = {
          map: pgRange => {
            const parsed = pgRangeParser.parse(pgRange); // Since the value we will get from `parsed.(start|end).value` is a
            // string but our code will expect it to be the value after `pg`
            // parsed it, we pass through to `pg-types` for parsing.

            const pgParse = rawTypes.indexOf(parseInt(subtype.id, 10)) >= 0 ? identity : pgTypes.getTypeParser(subtype.id);
            const {
              start,
              end
            } = parsed;
  ...
  builder.hook("build", build => {
    ...
    const GraphQLHStoreType = makeGraphQLHstoreType(graphql, hstoreTypeName); // const GraphQLHStoreType = build.getTypeByName(pgLegacyJsonUuid ? "Json" : "JSON");
