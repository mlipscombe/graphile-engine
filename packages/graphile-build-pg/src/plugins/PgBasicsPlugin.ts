...
import { PgProc, PgType, PgClass, PgAttribute, PgConstraint, PgEntity } from "./PgIntrospectionPlugin";
...
import baseOmit, { CREATE, READ, UPDATE, DELETE, ALL, MANY, ORDER, FILTER, EXECUTE } from "../omit";
...
const omitWithRBACChecks = omit => (entity: PgProc | PgClass | PgAttribute | PgConstraint, permission: string) => {
  ...
  if (entity.kind === "procedure") {
    ...
    if ((permission === READ || permission === ALL || permission === MANY) && !tableEntity.aclSelectable && !tableEntity.attributes.some(attr => attr.aclSelectable)) {
      return true;
    } else if (permission === CREATE && !tableEntity.aclInsertable && !tableEntity.attributes.some(attr => attr.aclInsertable)) {
      return true;
    } else if (permission === UPDATE && !tableEntity.aclUpdatable && !tableEntity.attributes.some(attr => attr.aclUpdatable)) {
      return true;
    ...
    if (klass.aclSelectable || klass.attributes.some(attr => attr.aclSelectable)) {
      ...
      if ((permission === READ || permission === FILTER || permission === ORDER) && !attributeEntity.aclSelectable) {
        return true;
...
const omitUnindexed = omit => (entity: PgProc | PgClass | PgAttribute | PgConstraint, permission: string) => {
...
function describePgEntity(entity: PgEntity, includeAlias = true) {
  const getAlias = !includeAlias ? () => "" : () => {
    const tags = pickBy(entity.tags, (value, key) => key === "name" || key.endsWith("Name"));

    if (Object.keys(tags).length) {
      return ` (with smart comments: ${chalk.bold(Object.keys(tags).map(t => `@${t} ${tags[t]}`).join(" | "))})`;
    }
  ...
  try {
    if (entity.kind === "constraint") {
      return `constraint ${chalk.bold(`"${entity.name}"`)} on ${describePgEntity(entity.class, false)}${getAlias()}`;
      ...
      return `${kind} ${chalk.bold(`"${entity.namespaceName}"."${entity.name}"`)}${getAlias()}`;
    } else if (entity.kind === "procedure") {
      return `function ${chalk.bold(`"${entity.namespaceName}"."${entity.name}"(...args...)`)}${getAlias()}`;
    } else if (entity.kind === "attribute") {
      return `column ${chalk.bold(`"${entity.name}"`)} on ${describePgEntity(entity.class, false)}${getAlias()}`;
    }
  } catch (e) {
...
export default (function PgBasicsPlugin(builder, {
  ...
  builder.hook("inflection", (inflection, build) => {
    return build.extend(inflection, preventEmptyResult({
      ...
      _columnName(attr: PgAttribute, _options?: {
        skipRowId?: boolean;
      }) {
      ...
      functionMutationResultFieldName(proc: PgProc, gqlType, plural: boolean = false, outputArgNames: Array<string> = []) {
      ...
      functionOutputFieldName(proc: PgProc, outputArgName: string, index: number) {
      ...
      computedColumnList(pseudoColumnName: string, proc: PgProc, _table: PgClass) {
        return proc.tags.fieldName ? proc.tags.fieldName + "List" : this.camelCase(`${pseudoColumnName}-list`);
      },

      singleRelationByKeys(detailedKeys: Keys, table: PgClass, _foreignTable: PgClass, constraint: PgConstraint) {
      ...
      singleRelationByKeysBackwards(detailedKeys: Keys, table: PgClass, _foreignTable: PgClass, constraint: PgConstraint) {
      ...
      manyRelationByKeys(detailedKeys: Keys, table: PgClass, _foreignTable: PgClass, constraint: PgConstraint) {
        if (constraint.tags.foreignFieldName) {
          return constraint.tags.foreignFieldName;
        }
      ...
      manyRelationByKeysSimple(detailedKeys: Keys, table: PgClass, _foreignTable: PgClass, constraint: PgConstraint) {
      ...
      rowByUniqueKeys(detailedKeys: Keys, table: PgClass, constraint: PgConstraint) {
        if (constraint.tags.fieldName) {
          return constraint.tags.fieldName;
        }
      ...
      updateByKeysInputType(detailedKeys: Keys, table: PgClass, constraint: PgConstraint) {
        if (constraint.tags.updateFieldName) {
      ...
      deleteByKeysInputType(detailedKeys: Keys, table: PgClass, constraint: PgConstraint) {
        if (constraint.tags.deleteFieldName) {
